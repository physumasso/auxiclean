...
with open('Oxi-Clean - Students.csv') as students_data :
	next(students_data)
...
with open('Oxi-Clean - Cours.csv') as class_data :
	next(class_data)
				...
				if he_is_placed == False :
					for i in range(1, 7) :
						if he_is_placed == False :
							...
							if results == None :
								if i == 6 :
									egalite = input("Pour le cours %s, choisir le gagnant entre 1 : %s et 2 : %s : " % (Nom_du_cours, Candidats[0], person[0]))
		...
		for Students in Liste_des_etudiants :
			if Students.Choix[0][1:] == Cours.Code :
				if Students.Dispo != "0" :
					...
					Cours.Etudiants.append([Students.Name, int(Students.Choix[0][1]), int(Students.Tp), int(Students.Scholarite), int(Students.Nobel), int(Students.Programme), int(Students.Cote)])
